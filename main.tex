...
\begin{definition}{Short circuit current}{scc}
    The finite slope of the input signal causes a direct current path between \(V_{DD}\) and GND for a short period of time during switching when both the \gls{nmos} and \gls{pmos} transistors are conducting.
\end{definition}
...
\end{description}
\(C_{\text{testing}}\) and \(C_{\text{packaging}}\) are fixed once the \gls{ic} is made; they are mostly a function of the \gls{ic}'s complexity and \textbf{NOT of the manufacturing process}. Meanwhile:
\[
...
\begin{definition}{MOPS and MFLOPS}{mops-mflops-def}
    To solve the consistency problems of \gls{mips}, performance can be measured using strictly defined mathematical operations:
    \begin{align*}
...
\paragraph{The \gls{cpu} Performance Equation}
...
\noindent The CPI can be furthermore calculated, also before the implementation of the actual infrastructure with this formula:
\[
    \text{CPI} = \frac{\sum_{i=1}^n\text{IC}_i\cdot\text{CPI}_i}{\text{IC}}
\]
Where:
\begin{description}
    \item[\(\text{IC}\)] Is the instruction complexity, with \(i\) referring to the single instruction
\end{description}
\paragraph{The choiche of the Task}
An important choice that we have to make to use execution time as a metrics is the task to execute, or better known as witch benchmark shall we execute to test the architecture. We distinguish different types of beanchmarks:

\begin{itemize}
    \item Real Application
    \item Modified (scripted) Application
    \item Kernels
    \item Synthetic benchmarks (e.g., Dhrystone, Whetstone)
    \item Toy benchmarks
\end{itemize}
The selection of the benchmark suffers from a problem. Companies producing architectures trends to optimize the benchmark to highlight better performance of their product. To adress (but not solve) this problem some action are taken:
\begin{itemize}
    \item Suites of multiple application are used;
    \item Benchmark source code modification, generally, are not allowed;
    \item System configuration and compilers option \textbf{must} be reported ammong the mesured data;
\end{itemize}
Also different class of use for an architecture requires a different type of beanchmark, we recognise Desktop Computing defined by SPEC, Servers defined by TPC and Embedded Computing defined by EEMBC. The suites defined by these corporation changes in the yerars to reflect current technologies and prevent excessive sensitivity to optimizations by the producers. 
...
% Force all acronyms to print, regardless of whether they were cited
\glsaddall
