...
\noindent This formally proves that the set of all decision functions has the exact same cardinality as the power set \(\mathcal{P}(\Sigma^*)\). By Cantor's theorem, the power set of a countably infinite set (like \(\Sigma^*\), \cref{def:strings}) is strictly larger and therefore uncountable, which mathematically reinforces our previous result (\cref{thm:uncount-dec}).
